...
module.exports = function(options) {
  ...
  return function(core) {
    ...
    function buildSQL(entity, query, params) {
      ...
      function processPart(part, prefix) {
        if (part.op) {
          paramIndex++
          if (part.op === 'like') {
            like = part.field
      ...
      if (query.join && query.type !== 'count') {
        ...
        query.join.forEach(function(join) {
          ...
          if (field.relationship.indexOf('one-') === 0) {
            ...
            if (join.having) {
              sql += ' LEFT JOIN '+entty.id+' ON '+entity.id+'_'+field.id+'='+entty.id+'._id'
              sql += ' WHERE '+processPart(join.having, entty.id+'.')
            }
      ...
      if (query.where && query.type !== 'read') {
      ...
      select += tables.map(function(table) {
        return expandColumnNames(table.entity, table.prefix, table.field)
      }).join(', ')
      return select+sql
    }
    ...
    db.list = function(args, callback) {
      ...
      txain(connect)
        ...
        .then(function(conn, query, callback) {
          ...
          query.type = 'count'
          var count = buildSQL(entity, query, params2)
    ...
    db.readObjects = function(args, callback) {
      ...
      txain(connect)
        ...
        .then(function(result, callback) {
          ...
          return callback(null, normalizeObjects(entity, result).objects)
        })
        .end(callback)
    }
    ...
    function expandColumnNames(entity, prefix, field) {
      var fields = entity.fields

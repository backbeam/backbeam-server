  ...
  return function(core) {
    ...
    function buildSQL(entity, query, params, where) {
      ...
      function processPartOp(entity, part, prefix) {
        if (part.op === 'like') {
          like = part.field
          return 'MATCH ('+columnName(part.field, prefix)+') AGAINST (?)'
        } else if (part.op === 'in' || part.op === 'not in') {
          if (part.field === 'this' && part.prop) {
            ...
            return columnName(part.field, prefix)+' '+part.op+' (?)'
          }
        } else {
          var op = part.op === 'is' ? '=' : part.op
          return columnName(part.field, prefix)+' '+op+' ?'
        }
      }
      ...
      if (query.join && query.type !== 'count') {
        var t = 1
        query.join.forEach(function(join) {
          ...
          if (field.relationship.indexOf('one-') === 0) {
            ...
            sql += ' INNER JOIN (SELECT '+innerFields+' FROM '+relationship
            var subquery = ''
            if (join.having) {
      ...
      entities.forEach(function(entity) {
        ...
        fields.forEach(function(field) {
          if (field.type === 'reference' && field.relationship.indexOf('many-') === 0) {
            ...
            var currentTableA = _.find(current, function(tble) {
              if (entity.id === tble.name || (entity.formerly || []).indexOf(tble.name) > -1) {
                return tble.name
              }
            })
            var currentTableB = _.find(current, function(tble) {
              if (entty.id === tble.name || (entty.formerly || []).indexOf(tble.name) > -1) {
                return tble.name
              }
            })
    ...
    db.migrateSchema = function(run, callback) {
      ...
      function compareTable(table, conn, current, commands, callback) {
        var currentTable = _.find(current, function(tble) {
          if (table.name === tble.name || (table.formerly || []).indexOf(tble.name) > -1) {
            return tble
          }
        })
      ...
      function compareColumn(conn, table, column, currentTable, commands, callback) {
        var currentColumn = _.find(currentTable.columns, function(colmn) {
          if (column.name === colmn.name || (column.formerly || []).indexOf(colmn.name) > -1) {
            return colmn
          }
        })
      ...
      function compareIndex(conn, table, index, currentTable, commands, callback) {
        ...
        var currentIndex = _.find(currentTable.indexes, function(indx) {
          if (index.name === indx.name || (index.formerly || []).indexOf(indx.name) > -1) {
            return indx
          }
        })
        if (currentIndex

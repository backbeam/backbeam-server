...
module.exports = function(options) {
  ...
  var fulltext = manager.supportsFullText
        ...
        if (part.op === 'like') {
          ...
          if (fieldname === 'this' && part.prop) {
            ...
            return columnName('_id', prefix)+' IN('+
              'SELECT '+entty.id+'_'+field.id+' FROM '+quote(relationship)+' WHERE '+entity.id+'_'+field.inverse+'=?)'
          } else {
      ...
      if (query.join) {
        ...
        query.join.forEach(function(join) {
          ...
          if (field.relationship.indexOf('one-') === 0) {
            sql += ' LEFT JOIN '+quote(field.entity)+' '+quote(prefix)+' ON t.'+field.id+'='+prefix+'._id'
            prefix += '.'
            tables.push({
              entity: entty,
              prefix: prefix,
            ...
            var innerFields = [field.entity+'_'+field.inverse, entity.id+'_'+field.id, '_created_at'].map(function(column) {
              return relationship+'.'+column+' AS '+quote(relationship+'.'+column)
            }).join(', ')
            sql += ' LEFT JOIN (SELECT '+innerFields+' FROM '+quote(relationship)
            var subquery = ''
            if (join.having) {
              subquery += ' LEFT JOIN '+quote(entty.id)+' ON '+entity.id+'_'+field.id+'='+entty.id+'._id'
              subquery += ' WHERE '+processPart(entty, join.having, entty.id+'.', true)
            }
            sql += subquery
            subquery = 'SELECT COUNT(*) FROM '+quote(relationship)+subquery
            var order = join.op === 'last' ? 'DESC' : 'ASC'
            sql += ' ORDER BY '+quote(relationship+'._created_at')+' '+order
            if (join.n) {
              sql += ' LIMIT '+(+join.n)
            }
            sql += ') AS '+quote(relationship)
            sql += ' ON t._id='+quote(relationship+'.'+field.entity+'_'+field.inverse)
            ...
            sorts.push(quote(relationship+'._created_at')+' '+order)

            sql += ' LEFT JOIN '+quote(entty.id)+' '+quote(prefix)+' ON '+quote(relationship+'.'+entity.id+'_'+field.id)+'='+prefix+'._id'
          }
          if (join.fetch) {
          ...
          if (field.type === 'reference') {
            ...
            if (field.relationship.indexOf('many-') === 0) {
              ...
              value.forEach(function(value) {
                if (typeof value !== 'undefined') {
                  sqlcommands.push('DELETE FROM '+quote(relationship)+' WHERE '+column1+'=? AND '+column2+'=?')
                  allparams.push([value, id])

                  sqlcommands.push('INSERT INTO '+quote(relationship)+' ('+column1+', '+column2+', _created_at, _updated_at) VALUES (?, ?, ?, ?)')
      ...
      .then(function(conn, callback) {
        ...
        entity.fields.forEach(function(field) {
          if (field.type === 'reference') {
            ...
            if (field.relationship.indexOf('many') >= 0) {
              var relationship = tableForRelationship(entity, field)
              sql.push('DELETE FROM '+quote(relationship)+' WHERE '+quote(field.entity+'_'+field.inverse)+' = ?')
              params.push([id])
            }
          }
        })

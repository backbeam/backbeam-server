...
module.exports = function(options) {
  ...
  return function(core) {
    ...
    function buildSQL(entity, query, params) {
      ...
      function processPartOp(part, prefix) {
        if (part.op === 'like') {
          like = part.field
      ...
      function processPart(part, prefix, prepend) {
        if (part.op) {
          paramIndex++
          var result = processPartOp(part, prefix)
          if (prepend) {
          ...
          var parts = _.map(part.constraints, function(constraint) {
            return processPart(constraint, prefix, prepend)
          })
        ...
        query.join.forEach(function(join) {
          ...
          if (field.relationship.indexOf('one-') === 0) {
            ...
            if (join.having) {
              ...
              subquery += ' WHERE '+processPart(join.having, entty.id+'.', true)
            }
            sql += subquery

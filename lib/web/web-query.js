...
module.exports = function(core, sandbox, req, res, private) {
  ...
  function sendObjects(err, objects, ids, count) {
    if (err) {
      return sendControllerError(err)
    }
  ...
  function createQuery(entity) {
    ...
    q.remove = function() {
      ...
      if (_callback === sandbox.response) {
        var callback = wrap(function(err, removed) {
          if (err) { return sendControllerError(err) }
    ...
    q.fetch = function() {
      ...
      if (_callback === sandbox.response) {
        var callback = wrap(function(err, ids, objects, count) {
          if (err) return private.sendControllerError(err)
          sandbox.response.json({
            status:'Success',
            objects: utils.denormalizeDictionary(objects),
            ids: ids,
    ...
    q.near = function() {
      ...
      if (_callback === sandbox.response) {
        var callback = wrap(function (err, ids, objects, count, distances) {
          if (err) { return sendControllerError(err) }
          sandbox.response.json({ status:'Success', objects:utils.denormalizeDictionary(objects), ids:ids, count:count, distances:distances })
        })
    ...
    q.bounding = function() {
      ...
      if (_callback === sandbox.response) {
        var callback = wrap(function (err, ids, objects, count) {
          if (err) { return sendControllerError(err) }
          sandbox.response.json({ status:'Success', objects:utils.denormalizeDictionary(objects), ids:ids, count:count })
        })

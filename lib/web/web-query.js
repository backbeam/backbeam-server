...
module.exports = function(core, req, res, private) {
  ...
  backbeam.set = function() {
    var args  = argmnts(arguments)
    var field = args.nextString('field')
    var value = args.rest()[0]

    backbeamSettings[field] = value
  }

  backbeam.get = function() {
    var args  = argmnts(arguments)
    var field = args.nextString('field')

    return backbeamSettings[field]
  }

  backbeam.select = function() {
  ...
  function createQuery(entity) {
    ...
    q.limit = function(limit) {
      _limit = limit
      return this
    }

    q.offset = function(offset) {
      _offset = offset
      return this
    }
    ...
    q.fetch = function() {
      var args      = argmnts(arguments, private.response)
      var limit     = args.nextNumber('limit', true) || _limit
      var offset    = args.nextNumber('offset', true) || _offset
      var callback  = args.callback()
      ...
      _fields.forEach(function(field) {
        ...
        if (type === 'reference') {
          if (references && _.isString(value)) {
            value = references[value] || backbeam.empty(object.entity(field), value)

...
module.exports = function(core, req, res, private) {
  ...
  response.json = function(object) {
    if (typeof object === 'string') {
      ...
      var callbackname = private.request.query.callback
      if (callbackname) {
  ...
  response.render = function() {
    ...
    try {
      var filters = private.libs.require('filters.js')
    } catch (err) {
      console.log('err', err.stack)
      // ignore if it's ENOENT error. That file is not required
    }
    ...
    function renderJade(callback) {
      ...
      if (core.isDevelopment()) {
        ...
        options.interactiveScript = req.interactiveScript
      } else if (core.isProduction()) {
        options.cache = true
      }
      try {

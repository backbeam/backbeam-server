...
module.exports = function(core, sandbox, req, res, private) {
  ...
  sandbox.backbeam.login = function() {
    ...
    var callback = wrap(function(err, id, references, authCode) {
    ...
    core.users.login({ email:email, password:password }, joins, params, callback)
  }
  ...
  sandbox.backbeam.requestPasswordReset = function() {
    ...
    core.users.lostPassword(email, callback)
  }
  ...
  sandbox.backbeam.resetPassword = function() {
    ...
    var callback = wrap(function(err, _user, authCode) {
      if (handleError(err, callback, _callback)) return
      var user = null
      if (_user) {
        var id = _user._id
    ...
    core.users.setPassword(code, password, callback)
  }
  ...
  sandbox.backbeam.verifyCode = function() {
    ...
    var callback = wrap(function(err, user, references, authCode) {
      if (handleError(err, callback, _callback)) return
      if (user) {
        var objects = objectsFromValues(references, null)
    ...
    core.users.verifyCode(code, joins, params, callback)
  }
